...
* ConfigDB Watcher: where the client is triggered by a Flow entity in the configuration database with a 'COMPLETED' state.
* Kafka Watcher (scheduled for deprecation): where the client is triggered by incoming kafka messages on a specified topic
...
**Note: The Kafka Watcher is scheduled for deprecation and will be superseded by the ConfigDB Watcher.**
