...
UNKNOWN_EB_ID = "dlm-generated-ebid"
...
def test_data_product_metadata(tmp_path):
    ...
    with pytest.raises(FileNotFoundError):
        DataProductMetadata(f"{tmp_path}/a-dummy-file", create_metadata=True)
    ...
    dpm = DataProductMetadata(absolute_path_to_dp_file, create_metadata=True)
    ...
    dpm = DataProductMetadata(absolute_path_to_dp_dir, create_metadata=True)
    assert dpm.get_execution_block_value().startswith(UNKNOWN_EB_ID) is True
