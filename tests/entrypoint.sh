...
python3 -m ska_dlm_client.directory_watcher.main --directory-to-watch /dlm/watch_dir --storage-name dlm-watcher --storage-root-directory /dlm/watch_dir --use-polling-watcher --ingest-server-url "http://dlm_ingest:8001" --migration-server-url http://dlm_migration:8004 --migration-destination-storage-name data --readiness-probe-file "/tmp/dlm-client-ready" --skip-rclone-access-check-on-register
