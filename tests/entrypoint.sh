...
case "$1" in
    "directory-watcher")
        ...
        ;;
    "configdb-watcher")
        CMD="SDP_CONFIG_HOST='etcd' dlm-configdb-watcher --source-name \${SOURCE_NAME:-sdp-watcher} --source-root \${SOURCE_ROOT:-/dlm/testing1} --target-name \${TARGET_NAME:-dlm-archive} --migration-url \${MIGRATION_URL:-http://dlm_migration:8004}"
        ;;
    *)
